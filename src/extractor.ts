...
interface BrowserSnapshot {
  pageSize: LayoutSnapshot["pageSize"];
  blocks: ContentBlock[];
  images: ImageInfo[];
  domTree: LayoutSnapshot["domTree"];
  plainText: string;
  markdown: string;
  larkXml?: string;
  gifAnchors?: LarkGifAnchor[];
  headings: ContentBlock[];
  links: Array<{ text: string; href: string; domPath: string }>;
  stylesheets: Array<{ href: string | null; text?: string; disabled?: boolean }>;
...
async function collectSnapshot(page: Page, includeStyles: boolean, includeHtml: boolean, maxDepth: number): Promise<BrowserSnapshot> {
  const snapshot = await page.evaluate(
    ({
      ...
      type StyleSummary = Record<string, string>;
      type SnapshotBlock = Record<string, unknown>;
      type SnapshotImage = Record<string, unknown>;
      ...
      function bounds(el: Element): Bounds {
      ...
      function styles(el: Element): StyleSummary | undefined {
        if (!shouldIncludeStyles) return undefined;
      ...
      function attributes(el: Element): Record<string, string> {
        return Array.from(el.attributes).reduce<Record<string, string>>((acc, attr) => {
      ...
      function domPath(el: Element): string {
        ...
        while (current && current.nodeType === Node.ELEMENT_NODE) {
          ...
          const sameTagSiblings = Array.from(parent.children as HTMLCollectionOf<Element>).filter((child) => child.tagName === currentTag);
      ...
      function blockType(tag: string): string {
        ...
        if (tag === "p" || tag === "figcaption") return "paragraph";
        ...
        if (tag === "section" || tag === "article" || tag === "main" || tag === "header" || tag === "footer" || tag === "nav" || tag === "aside") return "section";
        return "other";
      }
      ...
      function contentScore(el: Element): number {
        if (!visible(el) || isNoiseElement(el)) return -Infinity;
      ...
      function selectContentRoot(): Element {
        const selectors = [
          "article",
          "main",
          "[role='main']",
      ...
      function imageSource(el: Element): string {
        if (el instanceof HTMLImageElement) return el.currentSrc || el.src || el.getAttribute("data-src") || "";
      ...
      function inlineMarkdown(node: Node): string {
        ...
        if (!(node instanceof Element) || !visible(node) || skippedTags.has(node.tagName.toLowerCase())) return "";
      ...
      function directInlineMarkdown(el: Element): string {
      ...
      function renderMarkdown(el: Element, listDepth = 0): string[] {
        if (!visible(el) || skippedTags.has(el.tagName.toLowerCase()) || isNoiseElement(el)) return [];
        const tag = el.tagName.toLowerCase();
        ...
        if (tag === "div" && className.includes("ace-line")) {
          ...
          if (!text) return [];
          if (className.includes("heading-h2")) return [`## ${text}`];
          if (className.includes("heading-h3")) return [`### ${text}`];
          if (className.includes("heading-h4")) return [`#### ${text}`];
          if (className.includes("list-div") && /(?:^|\s)ol-/.test(className)) return [`1. ${text}`];
          if (className.includes("list-div")) return [`- ${text}`];
          return [text];
        }
        if (/^h[1-6]$/.test(tag)) {
          const text = inlineMarkdown(el);
        ...
        if (tag === "pre") return [`\`\`\`\n${el.textContent?.trim() ?? ""}\n\`\`\``];
        if (tag === "table") return [renderTable(el)].filter(Boolean);
        if (tag === "img") {
          const text = inlineMarkdown(el);
          return text ? [text] : [];
        }
        ...
        if ((tag === "div" || tag === "section") && !el.querySelector(blockChildSelector)) {
          ...
          if (el.querySelector("a[href]") && text.length < 120) return [];
          return text && text.length > 8 ? [text] : [];
        }
        return Array.from(el.children).flatMap((child) => renderMarkdown(child, listDepth));
      }

      function listMetaFromAceLine(el: Element): { kind: "ol" | "ul" | "indent"; level: number; text: string } | undefined {
        const className = String(el.className || "");
        if (!className.includes("list-div")) return undefined;
      ...
      function isGifImageSource(src: string): boolean {
        try {
      ...
      function cleanImageCaption(value: string | null | undefined): string | undefined {
        const text = value?.trim();
        if (!text || text === "图片" || /^GIF 图片\s*\d*$/i.test(text)) return undefined;
        return text;
      }
      ...
      function renderLarkXml(root: Element, title: string): { xml: string; gifAnchors: Array<{ anchor: string; src: string; caption?: string; order: number }> } {
        ...
        if (!aceLines.length) {
          for (const line of renderMarkdown(root)) {
            if (/^###\s+/.test(line)) parts.push(`<h3>${xmlEscape(line.replace(/^###\s+/, ""))}</h3>`);
            else if (/^##\s+/.test(line)) parts.push(`<h2>${xmlEscape(line.replace(/^##\s+/, ""))}</h2>`);
            else if (/^#\s+/.test(line)) parts.push(`<h1>${xmlEscape(line.replace(/^#\s+/, ""))}</h1>`);
            else if (/^-\s+/.test(line)) parts.push(`<ul><li>${xmlEscape(line.replace(/^-\s+/, ""))}</li></ul>`);
        ...
        type ListLine = { kind: "ol" | "ul" | "indent"; level: number; text: string };
        type ListItemNode = { text: string; children: ListBlockNode[] };
        type ListBlockNode = { type: "list"; kind: "ol" | "ul"; items: ListItemNode[] } | { type: "p"; text: string };
        const listGroup: ListLine[] = [];
        const renderListBlocks = (blocks: ListBlockNode[], inList = false): string =>
          blocks
            .map((block) => {
              if (block.type === "p") return inList ? `<br/>${block.text}` : `<p>${block.text}</p>`;
            ...
            .join("");
        const renderListGroup = (lines: ListLine[]): string => {
          const root: ListBlockNode[] = [];
          const stack: Array<{ container: Extract<ListBlockNode, { type: "list" }>; item: ListItemNode } | undefined> = [];
          ...
          for (const line of lines) {
            ...
            if (line.kind === "indent") {
              ...
              const paragraph: ListBlockNode = { type: "p", text: line.text };
              if (target) target.children.push(paragraph);
              else root.push(paragraph);
              continue;
            }

            for (let index = level + 1; index < stack.length; index += 1) stack[index] = undefined;
            ...
            const container =
              current && current.kind === line.kind && parentChildren[parentChildren.length - 1] === current
                ? current
                : ({ type: "list", kind: line.kind, items: [] } satisfies Extract<ListBlockNode, { type: "list" }>);
            if (parentChildren[parentChildren.length - 1] !== container) parentChildren.push(container);

            const item: ListItemNode = { text: line.text, children: [] };
        ...
        for (const el of aceLines) {
          if (!visible(el) || isNoiseElement(el)) continue;
          const className = String(el.className || "");
      ...
      function inlineRuns(el: Element): Array<Record<string, unknown>> {
        const runs: Array<Record<string, unknown>> = [];
        const walker = document.createTreeWalker(el, NodeFilter.SHOW_TEXT, {
          acceptNode(node) {
            if (!node.textContent?.trim()) return NodeFilter.FILTER_REJECT;
            const parent = node.parentElement;
            if (!parent || skippedTags.has(parent.tagName.toLowerCase())) return NodeFilter.FILTER_REJECT;
      ...
      function nearbyText(el: Element, direction: "previous" | "next"): string | undefined {
      ...
      function imageFromElement(el: Element, kind: string, src: string): SnapshotImage {
      ...
      function meaningfulImage(el: Element, kind: string): boolean {
        const rect = el.getBoundingClientRect();
        if (kind === "svg" && (rect.width < 48 || rect.height < 48)) return false;
        if (kind === "background" && (rect.width < 80 || rect.height < 80)) return false;
      ...
      function blockFromElement(el: Element, depth: number): SnapshotBlock | null {
        const tag = el.tagName.toLowerCase();
        if (skippedTags.has(tag) || !visible(el)) return null;
        if (!blockTags.has(tag) && tag !== "img" && tag !== "iframe" && tag !== "video" && tag !== "audio") return null;

        const text = normalizeText(el.textContent ?? "");
        const type = blockType(tag);
        if (type !== "image" && type !== "embed" && !text) return null;
      ...
      function buildDomTree(node: Element, depth = 0): Record<string, unknown> | null {
        const tag = node.tagName.toLowerCase();
        if (skippedTags.has(tag) || depth > depthLimit) return null;
        return {
          id: nextId("dom"),
          tag,
          type: "element",
  ...
  return snapshot as unknown as BrowserSnapshot;
}
...
function cleanDefaultImageCaption(value: string | undefined): string | undefined {
  const text = value?.trim();
  if (!text || text === "图片" || /^GIF 图片\s*\d*$/i.test(text)) return undefined;
  return text;
}

function applyGifAnchorsFromCachedImages(xml: string | undefined, existingAnchors: LarkGifAnchor[] | undefined, images: ImageInfo[]): { xml?: string; gifAnchors?: LarkGifAnchor[] } {
  if (!xml) return { xml, gifAnchors: existingAnchors };

  let result = xml;
  const gifAnchors = [...(existingAnchors ?? [])];
  const alreadyAnchored = new Set(gifAnchors.map((anchor) => anchor.src));
  for (const image of images) {
    if (alreadyAnchored.has(image.absoluteUrl) || (!isGifPathOrUrl(image.localPath) && !isGifPathOrUrl(image.absoluteUrl))) continue;
    ...
    const caption = cleanDefaultImageCaption(
      decodeXmlAttribute(match[0].match(/\bcaption="([^"]*)"/)?.[1] ?? image.caption ?? image.alt ?? image.title ?? "")
    );
...
async function cacheImages(browser: Browser, outputDir: string, images: ImageInfo[]): Promise<string | undefined> {
  if (images.length === 0) {
    return undefined;
  }
...
function updatePreviewImageRefs(data: WebpageExtraction): void {
...
function normalizeWaitUntil(waitUntil: WaitUntil | undefined): "load" | "domcontentloaded" | "networkidle" | "commit" {
  return waitUntil ?? "networkidle";
}

export async function extractWebpage(options: ExtractOptions): Promise<WebpageExtraction> {
  ...
  let primarySnapshot: BrowserSnapshot | undefined;
...
export function createRunId(url: string): string {
