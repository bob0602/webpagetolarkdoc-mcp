...
server.tool("extract_webpage_content", "完整解析网页文本、图片、样式、DOM 层级、视觉布局，并生成结构化 JSON 与可交互预览 HTML。", extractSchema, async (args) => {
...
server.tool("sync_webpage_to_lark_doc", "解析指定网页并同步到飞书云文档：先检查目标文档是否存在，不存在则创建，存在则按 append/overwrite 更新。", syncSchema, async (args) => {
    const outputDir = args.outputDir ?? path.resolve("output", createRunId(args.url));
    const data = await extractWebpage({
        url: args.url,
        outputDir,
...
server.tool("describe_output_schema", "查看网页解析结果 JSON 的顶层数据结构说明。", {}, async () => ({
    content: [
        {
            type: "text",
            text: JSON.stringify({
